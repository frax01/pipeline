...
def main(start_idx: int, end_idx: int = None, reset: bool = False, excel_path: str = None):
    ...
    if reset or start_idx == 0:
        print(f"Resetting stats and logs for MCP Check NPX (start: {start_idx})")
    ...
    for idx in range(start_idx, end_idx):
        ...
        if not ram_ok:
            print(f"\n STOPPING: RAM critically high. Resume with: python run_check.py --start -1 --end {end_idx}")
            break
        ...
        print(f"[mcp-check-npx] {idx}/{end_idx} | nodejs | {_status} | {end_time - start_time:.2f}s | remaining: {remaining}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run MCP Check on NPX servers")
