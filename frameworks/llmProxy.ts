...
async function generatePromptsForTool(tool: any): Promise<GeneratedPrompt[]> {
...
function extractToolCall(text: string): { toolName: string; arguments: any } | null {
...
function normalizeToolCall(parsed: any): { toolName: string; arguments: any } | null {
  // Formato canonico
...
async function main() {
  ...
  for (const tool of toolsForPrompt) {
    ...
    let dynamicPrompts: GeneratedPrompt[] = []
