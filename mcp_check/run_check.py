...
# Initialization structure for mcp-check stats
INIT_STATS = {
    "last_index": 0,
    "total": 0,
    "range_start": 0,
...
def save_check_entry(server_url, server_data, framework_result):
    """Save granular check results to hierarchical folder structure, grouping by server."""
    out_dir = CURRENT_DIR
    server_name = server_data["server_name"]
    language = server_data["server_language"]
    
    suites = framework_result.get("suites", {})
    for suite_name, suite_data in suites.items():
        # Clean suite name (replace - with _)
        ...
        for category, err_list in cat_errors.items():
            ...
            # Use the first error message for slug
            ...
            if server_entry:
                ...
                current_data["entries"].append(server_entry)
            
            current_data["total"] = len(current_data["entries"])
            with open(target_file, "w", encoding="utf-8") as f:
                json.dump(current_data, f, indent=4, ensure_ascii=False)

        # Group Warnings by server
...
def cleanup_orphan_repos(base_dir: Path):
    ...
    keep = {
        ...
        "mcp_security_scan", "mcp_guard", "mcp_scan",
        "mcp_shield", "mcp_watch", "mcp_check",
        "fuzzing", "scanorama", "mcp_validator",
    }
    removed = 0
...
def main(start_idx: int, end_idx: int = None, reset: bool = False):
    ...
    if reset or start_idx == 0:
        ...
        # New: delete suite folders for fresh start
        for folder in ["handshake", "tool_discovery", "tool_invocation"]:
            ...
            if server_data is not None:
                ...
                try:
                    ...
                    if status == "completed":
                        ...
                        save_check_entry(server_url, server_data, framework_result["mcp-check"])
                    elif status == "failed":
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run MCP Check independently with hierarchical reporting")
