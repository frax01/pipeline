...
# Initialization structure for mcp-security-scan stats
INIT_STATS = {
    "last_index": 0,
    "total": 0,
    "range_start": 0,
...
def _update_category_file(base_dir: Path, category: str, severity: str, item: dict):
    """Aggiunge una vulnerabilità al file locale <code/category>_<severity>.json all'interno della cartella categoria"""
...
def update_output_files(server_url: str, server_name: str, mcp_security_data: dict):
    """Scrive i failed_findings separati per category e severity."""
    failed_findings = mcp_security_data.get("failed_findings", [])
...
def reset_all_output_files():
    """Cancella ricorsivamente tutte le cartelle delle categorie."""
...
def prepare_server(server_url: str) -> dict | None:
    ...
    # Build config (writes claude_desktop_config.json needed and builds logic)
    try:
...
def main(start_idx: int, end_idx: int = None, reset: bool = False):
    ...
    for idx, row in df_excel.iloc[start_idx:end_idx].iterrows():
        ...
        # ALWAYS update global FIRST (use real language even if scan failed)
        ...
        if server_data is not None and _status == "completed" and framework_result is not None:
            ...
            # Aggiorna le cartelle estraendo i failed_findings
            mcp_security_data = framework_result.get("mcp-security-scan", {})
            total_vulns = mcp_security_data.get("total-vulnerabilities", 0)
            
            if total_vulns > 0:
                update_output_files(server_url, server_data["server_name"], mcp_security_data)
